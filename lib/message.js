const parseHTML  = require('./parseHTML');
const { getChannelType }  = require('./channels');
const log = console.log;

class Message {
    constructor(chatID, title, descriprion) {
        this.chatType = getChannelType(chatID);
        this.title = title;
        this.descriprion = descriprion;
        this.attachments = [];
        
        for (let i = 3; i < arguments.length; i++) {
            this.attachments.push(arguments[i]);
        }
    }
}

const isUrl = function(msg) {
    return (msg.entities && Array.isArray(msg.entities) && msg.entities.length > 0 && msg.entities[0].type === 'url');
}

const logError = function(e){
    log('ERROR while creating message');
    log(e);
}

const emptyText = function(str, ifEmpty = '') {
    return (str||ifEmpty);
}

const createMessage = function(msg) {
    const chatID = msg.chat.id;
    if (isUrl(msg)) {
        return parseHTML(msg.text).then(
            parsed => {

                const desc = parsed.descriprion;
                const title = (desc) ? desc.substr(0, 100) : parsed.title;
                const descriprion = emptyText(parsed.title) + '\n' + emptyText(parsed.author) + '\n' + emptyText(parsed.descriprion);

                return new Message(
                    chatID,
                    title,
                    descriprion,
                    parsed.image,
                    parsed.url,
                );
            },
            e => logError(e)
        );
    } else {
        return new Promise((rs, rj) => rs(new Message(chatID, msg.text)) );
    }
}

module.exports = createMessage